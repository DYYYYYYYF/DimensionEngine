...
void HashTable::Create(size_t element_size, unsigned int element_count, void* memory, bool is_pointer) {
	if (memory == nullptr) {
		GLOG(Log::eError, "Hash table create failed. Pointer to memory or Hash memory is nullptr");
		return;
	}

	if (element_count == 0 || element_size == 0) {
		GLOG(Log::eError, "element_size and element_size must be a positive non-zero value.");
		return;
	}
...
void HashTable::Destroy() {
	Memory::Free(Memory, MemoryType::eMemory_Type_Hashtable);
}

bool HashTable::Set(const char* name, void* value) {
	if (name == nullptr || value == nullptr) {
		GLOG(Log::eError, "Hash table set falied. name or value is nullptr.");
		return false;
	}

	if (IsPointerType) {
		GLOG(Log::eError, "Hash table should not be used with tables that have pointer types.");
		return false;
	}
...
bool HashTable::Set(const char* name, void** value) {
	...
	if (!IsPointerType) {
		GLOG(Log::eError, "Hash table should not be used with tables that have non pointer types.");
		return false;
	}
...
bool HashTable::Get(const char* name, void* out_value) {
	if (name == nullptr || out_value == nullptr) {
		GLOG(Log::eError, "Hash table get falied. name or value is nullptr.");
		return false;
	}
...
bool HashTable::Fill(void* value) {
	if (value == nullptr || Memory == nullptr) {
		GLOG(Log::eError, "Hash table fill falied. value is nullptr.");
		return false;
	}

	if (IsPointerType) {
		GLOG(Log::eError, "Hash table fill should not be used with tables that have pointer types.");
		return false;
	}
