...
void InputProcessKey(Keys key, bool pressed) {
	if (state.keyboard_current.keys[key] != pressed) {
		...
		EventFire(pressed ? Event::eEvent_Code_Key_Pressed : Event::eEvent_Code_Key_Released, 0, context);
	}
}
...
void InputProcessButton(Buttons button, bool pressed) {
	if (state.mouse_current.buttons[button] != pressed) {
		...
		EventFire(pressed ? Event::eEvent_Code_Button_Pressed : Event::eEvent_Code_Button_Released, 0, context);
	}
}
...
void InputProcessMouseMove(short x, short y) {
	if (state.mouse_current.x != x || state.mouse_current.y != y) {
		...
		Event::EventFire(Event::eEvent_Code_Mouse_Moved, 0, context);
	}
}
...
void InputProcessMouseWheel(char z_delta) {
	...
	Event::EventFire(Event::eEvent_Code_Mouse_Wheel, 0, context);
}
